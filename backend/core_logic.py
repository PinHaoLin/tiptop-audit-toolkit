...
def get_project_root_dir():
    if getattr(sys, 'frozen', False):
        ...
        if os.path.isdir(os.path.join(parent_dir, "aud")) and os.path.isdir(os.path.join(parent_dir, REVIEW_FOLDER_NAME)):
            return parent_dir
...
def get_aud_dir_path():
    return os.path.join(get_project_root_dir(), "aud")
...
def modify_bat_scripts(*args, **kwargs):
    ...
    #  依據 bat.txt 範本完整重構內容，補上 topcust 區塊與 type 指令
    rpt_content = (
        f"@echo on\n\n"
...
def merge_diff_files_to_attachment(*args, **kwargs):
    ...
    for p in [path_4fd, path_4gl]:
        if os.path.exists(p):
            try:
                ...
                print(f" 讀取本地 aud Diff 檔案失敗 {os.path.basename(p)}: {e}", flush=True)
        else:
    ...
    if content_segments:
        ...
        print(f" 警告：在 {aud_dir_path} 下未找到任何 4gl_diff.txt 或 4fd_diff.txt 的內容或檔案為空。", flush=True)
...
def audit_and_compare_logs(*args, **kwargs):
    ...
    # =========================================================================
    #  步驟 2：使用 openpyxl 載入並讀取 Excel 紅色列資料
    # =========================================================================
    excel_red_rows_data = []
    try:
