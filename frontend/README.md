...
pyinstaller --clean --noconsole --onefile --icon logo.ico --add-data "../frontend/dist;web" app.py
封裝完成後，全新的獨立執行檔將生成於 backend/dist/app.exe，即可直接複製、建立桌面捷徑或發送給部門同事使用。

視窗模式說明： 後端啟動時會使用 Eel 的 Edge App 模式，使用者看到的是獨立應用程式視窗，而不是一般瀏覽器分頁。若同事電腦上的 Edge 被停用或移除，請先恢復 Edge/WebView 環境後再執行。
